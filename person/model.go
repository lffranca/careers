...
func getPeople(db *sql.DB, search string, hero string) ([]model.Person, error) {
	query := `
		select
			p.id,
			p.uuid,
	...
	args := []interface{}{}
	if hero != "" {
		isHero, errHero := strconv.ParseBool(hero)
		if errHero != nil {
			return nil, errHero
		}

		args = append(args, isHero)
		query += fmt.Sprintf(` and p.hero = $%d `, len(args))
	}

	if search != "" {
		args = append(args, fmt.Sprintf("%s%s%s", "%", search, "%"))

		if _, err := uuid.Parse(search); err == nil {
			args = append(args, search)
			query += fmt.Sprintf(` and (p.name ilike $%d or p.uuid = $%d) `, len(args)-1, len(args))
		} else {
			query += fmt.Sprintf(` and p.name ilike $%d `, len(args))
		}
	}

	rowsPerson, errRowsPerson := db.QueryContext(context.Background(), query, args...)
	if errRowsPerson != nil {
		return nil, errRowsPerson
	}
